...
class Email2FABackend extends TwoFactorAuthenticationBackend {
    ...
    function send($user) {
        ...
        if (!$cfg || !($info = $user->get2FAConfig($this->getId())))
            return new BaseError(/* @trans */ 'Unable to retrieve two factor authentication configuration');

        // Email account to send the OTP via
        if (!($email = $cfg->getAlertEmail() ?: $cfg->getDefaultEmail()))
            return new BaseError(/* @trans */ 'Unable to retrieve outgoing email account');

        // Recipient configured during 2FA setup
        $recipient = $info['config']['email'] ?: $user->getEmail();
        if (!$recipient)
            return new BaseError(/* @trans */ 'Unable to retrieve two factor authentication recipient');
       ...
       $msg = $ost->replaceTemplateVariables(array(
        ...
        if (!$email->send($recipient, Format::striptags($msg['subj']),
           $msg['body']))
            return new BaseError(/* @trans */ 'Unable to send two factor authentication email');
